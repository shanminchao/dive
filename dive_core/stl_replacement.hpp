...
//--------------------------------------------------------------------------------------------------
template<class Type>
Vector<Type>::Vector(uint64_t size, const Type& a)
{
    clear();
    resize(size, a);
}
...
// //--------------------------------------------------------------------------------------------------
// template<class Type> Vector<Type> &Vector<Type>::operator=(const Vector<Type> &a) const
// {
// }
...
template<class Type> void Vector<Type>::reserve(uint64_t size)
{
    if (size > m_reserved)
    {
        if (m_reserved == 0)
        {
            // Set a suitable initial size to prevent too many unnecessary re-allocations
            m_reserved = 1024;
        }
        else
        {
        ...
        // constructor
        void *raw_memory = operator new[](m_reserved * sizeof(Type));
        Type *new_buffer = (Type*)raw_memory;
        if (m_buffer != nullptr)
        {
            // std::copy uses copy constructors when applicable, and *should* result in a
            // memmove/memcpy for raw data
