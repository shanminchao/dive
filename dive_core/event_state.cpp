...
template<>
void EventStateInfoT<EventStateInfo_CONFIG>::Reserve(typename EventStateInfo::Id::basic_type new_cap)
{
    if (new_cap <= m_cap)
        return;
    ...
    uint32_t new_buffer_size = (num_bytes + sizeof(std::max_align_t)-1) / sizeof(std::max_align_t);
    ...
    // Copy all of the data from the old buffer to the new buffer
        static_assert(std::is_trivially_copyable<uint32_t>::value, "Field type must be trivially copyable");
        ...
        memcpy(PatchControlPointsPtr(), old_patch_control_points_ptr, kPatchControlPointsSize * m_size);
                static_assert(std::is_trivially_copyable<VkViewport>::value, "Field type must be trivially copyable");
        memcpy(ViewportPtr(), old_viewport_ptr, kViewportSize * m_size);
                static_assert(std::is_trivially_copyable<VkRect2D>::value, "Field type must be trivially copyable");
        ...
        memcpy(DepthClampEnabledPtr(), old_depth_clamp_enabled_ptr, kDepthClampEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(RasterizerDiscardEnabledPtr(), old_rasterizer_discard_enabled_ptr, kRasterizerDiscardEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<VkPolygonMode>::value, "Field type must be trivially copyable");
        memcpy(PolygonModePtr(), old_polygon_mode_ptr, kPolygonModeSize * m_size);
                static_assert(std::is_trivially_copyable<VkCullModeFlags>::value, "Field type must be trivially copyable");
        memcpy(CullModePtr(), old_cull_mode_ptr, kCullModeSize * m_size);
                static_assert(std::is_trivially_copyable<VkFrontFace>::value, "Field type must be trivially copyable");
        ...
        memcpy(DepthBiasEnabledPtr(), old_depth_bias_enabled_ptr, kDepthBiasEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<float>::value, "Field type must be trivially copyable");
        memcpy(DepthBiasConstantFactorPtr(), old_depth_bias_constant_factor_ptr, kDepthBiasConstantFactorSize * m_size);
                static_assert(std::is_trivially_copyable<float>::value, "Field type must be trivially copyable");
        memcpy(DepthBiasClampPtr(), old_depth_bias_clamp_ptr, kDepthBiasClampSize * m_size);
                static_assert(std::is_trivially_copyable<float>::value, "Field type must be trivially copyable");
        memcpy(DepthBiasSlopeFactorPtr(), old_depth_bias_slope_factor_ptr, kDepthBiasSlopeFactorSize * m_size);
                static_assert(std::is_trivially_copyable<float>::value, "Field type must be trivially copyable");
        memcpy(LineWidthPtr(), old_line_width_ptr, kLineWidthSize * m_size);
                static_assert(std::is_trivially_copyable<VkSampleCountFlagBits>::value, "Field type must be trivially copyable");
        memcpy(RasterizationSamplesPtr(), old_rasterization_samples_ptr, kRasterizationSamplesSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(SampleShadingEnabledPtr(), old_sample_shading_enabled_ptr, kSampleShadingEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<float>::value, "Field type must be trivially copyable");
        memcpy(MinSampleShadingPtr(), old_min_sample_shading_ptr, kMinSampleShadingSize * m_size);
                static_assert(std::is_trivially_copyable<VkSampleMask>::value, "Field type must be trivially copyable");
        memcpy(SampleMaskPtr(), old_sample_mask_ptr, kSampleMaskSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(AlphaToCoverageEnabledPtr(), old_alpha_to_coverage_enabled_ptr, kAlphaToCoverageEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        ...
        memcpy(DepthWriteEnabledPtr(), old_depth_write_enabled_ptr, kDepthWriteEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<VkCompareOp>::value, "Field type must be trivially copyable");
        memcpy(DepthCompareOpPtr(), old_depth_compare_op_ptr, kDepthCompareOpSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(DepthBoundsTestEnabledPtr(), old_depth_bounds_test_enabled_ptr, kDepthBoundsTestEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<float>::value, "Field type must be trivially copyable");
        ...
        memcpy(StencilTestEnabledPtr(), old_stencil_test_enabled_ptr, kStencilTestEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<VkStencilOpState>::value, "Field type must be trivially copyable");
        memcpy(StencilOpStateFrontPtr(), old_stencil_op_state_front_ptr, kStencilOpStateFrontSize * m_size);
                static_assert(std::is_trivially_copyable<VkStencilOpState>::value, "Field type must be trivially copyable");
        memcpy(StencilOpStateBackPtr(), old_stencil_op_state_back_ptr, kStencilOpStateBackSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(LogicOpEnabledPtr(), old_logic_op_enabled_ptr, kLogicOpEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<VkLogicOp>::value, "Field type must be trivially copyable");
        memcpy(LogicOpPtr(), old_logic_op_ptr, kLogicOpSize * m_size);
                static_assert(std::is_trivially_copyable<VkPipelineColorBlendAttachmentState>::value, "Field type must be trivially copyable");
        ...
        memcpy(LRZWritePtr(), old_lrz_write_ptr, kLRZWriteSize * m_size);
                static_assert(std::is_trivially_copyable<a6xx_lrz_dir_status>::value, "Field type must be trivially copyable");
        ...
        memcpy(LRZDirWritePtr(), old_lrz_dir_write_ptr, kLRZDirWriteSize * m_size);
                static_assert(std::is_trivially_copyable<a6xx_ztest_mode>::value, "Field type must be trivially copyable");
        ...
        memcpy(BinHPtr(), old_bin_h_ptr, kBinHSize * m_size);
                static_assert(std::is_trivially_copyable<uint16_t>::value, "Field type must be trivially copyable");
        ...
        memcpy(WindowScissorBRYPtr(), old_window_scissor_bry_ptr, kWindowScissorBRYSize * m_size);
                static_assert(std::is_trivially_copyable<a6xx_render_mode>::value, "Field type must be trivially copyable");
        memcpy(RenderModePtr(), old_render_mode_ptr, kRenderModeSize * m_size);
                static_assert(std::is_trivially_copyable<a6xx_buffers_location>::value, "Field type must be trivially copyable");
        memcpy(BuffersLocationPtr(), old_buffers_location_ptr, kBuffersLocationSize * m_size);
                static_assert(std::is_trivially_copyable<a6xx_threadsize>::value, "Field type must be trivially copyable");
        memcpy(ThreadSizePtr(), old_thread_size_ptr, kThreadSizeSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(EnableAllHelperLanesPtr(), old_enable_all_helper_lanes_ptr, kEnableAllHelperLanesSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(EnablePartialHelperLanesPtr(), old_enable_partial_helper_lanes_ptr, kEnablePartialHelperLanesSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(UBWCEnabledPtr(), old_ubwc_enabled_ptr, kUBWCEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(UBWCLosslessEnabledPtr(), old_ubwc_lossless_enabled_ptr, kUBWCLosslessEnabledSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(UBWCEnabledOnDSPtr(), old_ubwc_enabled_on_ds_ptr, kUBWCEnabledOnDSSize * m_size);
                static_assert(std::is_trivially_copyable<bool>::value, "Field type must be trivially copyable");
        memcpy(UBWCLosslessEnabledOnDSPtr(), old_ubwc_lossless_enabled_on_ds_ptr, kUBWCLosslessEnabledOnDSSize * m_size);
