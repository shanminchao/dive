            ...
            if (state_tracker.IsRegSet(rb_blit_info_offset))
            {
                ...
                else
                {
                    if (rb_blit_info.bitfields.GMEM)
                        type = SyncType::kSysMemToGmemBlit;
                    else
                        type = SyncType::kGmemToSysMemBlit;
                }
            }
        }
        return type;
                                 ...
                                 EmulateStateTracker  &state_tracker)
{
    ...
    DIVE_ASSERT(
    IsDrawDispatchResolveSyncEvent(mem_manager, submit_index, va_addr, opcode, state_tracker));
    ...
    else if (opcode == CP_EVENT_WRITE7)
    {
        ...
        {
            string_stream << "Blit(SysMem-To-Gmem)";
        }
        else if (sync_type == SyncType::kGmemToSysMemBlit)
        {
            string_stream << "Blit(GMem-To-Sysmem)";
        }
        else if (sync_type == SyncType::kClearGmem)
        {
            string_stream << "Blit(ClearGmem)";
        }
        else
        {
