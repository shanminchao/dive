...
#define GATHER_TOTAL_MIN_MAX_MEDIAN(array_name, type)                                     \
    {                                                                                     \
        ...
        {                                                                                 \
            uint32_t mid1 = array_name[n / 2 - 1];                                        \
            uint32_t mid2 = array_name[n / 2];                                            \
            stats_list[kMedian##type] = (float)(mid1 + mid2) / 2.0f;                      \
        }                                                                                 \
...
void GatherAndPrintStats(const Dive::CaptureMetadata &meta_data, std::ostream &ostream)
{
    ...
    const char *stats_desc[kNumStats] = {
        ...
        "Total vertices in all draws",
        "\tMin vertices in a single draw",
        "\tMax vertices in a single draw",
        "\tMedian vertices in a single draw",
    ...
    std::vector<uint32_t>           event_num_vertices;
    ...
    for (size_t i = 0; i < event_count; ++i)
    {
            ...
            (info.m_render_mode == Dive::RenderModeType::kBinning))
        {
            ...
            if (info.m_num_vertices != 0)
                event_num_vertices.push_back(info.m_num_vertices);
        }
